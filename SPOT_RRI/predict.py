...
import sys
import argparse
sys.path.append("/lustre/home/mlang/RRI_Model_ENV_v1/lib/python3.8/site-packages/")  # bio,tensorboardx
...
def run_spotrna(input_fa,out_path,job_name,ncpu):
    # usage: /lustre/home/xzhou/bin/SPOT-RNA -i INPUT -o OUTPATH -j JOBNAME -n NCPU
    #/lustre/home/xzhou/bin/SPOT-RNA
    if os.path.exists(out_path):
        pass
    else:
        os.makedirs(out_path)
    if os.path.exists(input_fa):
        cmd = "/lustre/home/xzhou/bin/SPOT-RNA -i {} -o {} -j {} -n {}".format(input_fa, out_path, job_name, ncpu)
        os.system(cmd)
    else:
...
def pair2dbn(predict_pair,seq1,seq2,rri_outputs):
    ...
    np.savetxt(os.path.join(rri_outputs,rri_id + '.dbn'), temp, delimiter='', fmt="%s",header='>' + 'RNA-RNA-Interaction', comments='')
...
if __name__ == '__main__':
    ...
    parser.add_argument("--job_name", type=str, metavar='')
    parser.add_argument("--ncpu", type=int, metavar='')
    parser.add_argument('--outputs', default='outputs/', type=str,
                        help='Path to output files; SPOT-RNAi outputs at least three files .ct, .bpseq, and .prob files; default = ''outputs/\n',metavar='')
